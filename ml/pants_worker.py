...
log = LoggerBuilder.get_logger(PANTS_WORKER_NAME_KEYWORD)

model = Pants_GAN()

class PantsWorker(BaseHTTPRequestHandler):
    ...
    def _inference_process(self, input_data: str, message_id: str) -> str:
        ...
        return model.inference(input_data, message_id, PANTS_SERVER_NAME)
...
def run(server_class=HTTPServer, handler_class=PantsWorker, port: int = 5001):
    server_address = ('', port)
