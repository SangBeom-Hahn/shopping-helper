...
log = LoggerBuilder.get_logger(HAT_WORKER_NAME_KEYWORD)

model = Hat_GAN()

class HatWorker(BaseHTTPRequestHandler):
    ...
    def _inference_process(self, input_data: str, message_id: str) -> str:
        ...
        return model.inference(input_data, message_id, HATS_SERVER_NAME)
...
def run(server_class=HTTPServer, handler_class=HatWorker, port: int = 5002):
    server_address = ('', port)
