...
log = LoggerBuilder.get_logger(SKIRT_WORKER_NAME_KEYWORD)

model = Skirt_GAN()

class SkirtWorker(BaseHTTPRequestHandler):
    ...
    def _inference_process(self, input_data: str, message_id: str) -> str:
        ...
        return model.inference(input_data, message_id, SKIRTS_SERVER_NAME)
...
def run(server_class=HTTPServer, handler_class=SkirtWorker, port: int = 5003):
    server_address = ('', port)
